...
export default function WitnessSection({ momentId, initialAngles }: WitnessSectionProps) {
  ...
  // تسجيل الشاهد (I Was Here)
  const handleWitness = async () => {
    if (hasWitnessed || witnessing) return;
    setWitnessing(true);
    try {
      const res = await fetch(`/api/moments/${momentId}/witness`, {
        method: 'POST',
      });
      if (res.ok) {
      ...
      console.error('Error recording witness:', err);
    } finally {
      setWitnessing(false);
    }
  };
  ...
  const handleAddAngle = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error adding angle:', err);
    } finally {
      setSubmitting(false);
    }
  };
      ...
      >
        {witnessing ? (
          ...
          <>
            ...
            You Confirmed You Were Here
          </>
        ) : (
          <>
            <CheckCircle2 className="w-4 h-4" />
            I Was Here / Confirm Witness
          </>
        )}
      </button>
      ...
      <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 space-y-3">
        ...
        <form onSubmit={handleAddAngle} className="space-y-2.5">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-100 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-all border border-slate-700"
          >
