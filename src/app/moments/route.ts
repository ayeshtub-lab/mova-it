...
// جلب اللحظات (GET) بالتنسيق الذي تتوقعه الصفحة الرئيسية
export async function GET() {
  ...
  try {
    ...
    // إرجاع البيانات داخل كائن يحتوي على success و data
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!title || !city || !areaName || !whyNowReason) {
    ...
    const values = [title, city, areaName, whyNowReason, missingPiece || null];
